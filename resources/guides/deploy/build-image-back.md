...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| 기술스택 정보 | `AGENTS.md` | 베이스 이미지 및 빌드 도구 결정 |
...
> `{version}`은 AGENTS.md의 `### develop > 기술스택 > Java` 버전을 참조하여 결정한다. 기술스택 정보가 없으면 기본값 `21`을 사용한다.
...
> `{version}`은 AGENTS.md의 `### develop > 기술스택 > Java` 버전을 참조하여 치환한다. 기본값: `amazoncorretto:21-alpine3.21-jdk`.
