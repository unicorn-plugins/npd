...
백엔드 서비스는 `./gradlew bootJar` (또는 `./mvnw package`)를 Docker 외부에서 실행하므로, VM에 JDK가 설치되어 있어야 한다.

```bash
# Java 설치 (Amazon Linux 2023 / AL2)
...
# 빌드 스크립트 실행 권한 부여
chmod +x gradlew   # Gradle 프로젝트
chmod +x mvnw      # Maven 프로젝트
```

> `{version}`은 CLAUDE.md의 `### develop > 기술스택 > Java` 버전을 참조한다. 기본값: `21`.
