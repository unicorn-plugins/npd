...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| 이미지 빌드 결과 | `deployment/container/build-image-back.md` | 푸시된 이미지 fullname 확인 |

## 출력 (이 단계 산출물)
| 산출물 | 파일 경로 |
|--------|----------|
| 컨테이너 실행 결과서 | `docs/deploy/run-container-back.md` |
...
`docs/deploy/run-container-back.md` 결과 보고서 템플릿:
...
- [ ] `docs/deploy/run-container-back.md`가 작성됨
