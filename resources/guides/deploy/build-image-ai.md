...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| 기술스택 정보 | `AGENTS.md` | 베이스 이미지 결정 (Python 버전) |
| AI 서비스 소스 | `(런타임 결정)` | 빌드 대상 |

## 출력 (이 단계 산출물)
| 산출물 | 파일 경로 |
|--------|----------|
| 컨테이너 이미지 빌드 가이드 | `docs/deploy/build-image-ai.md` |
...
> Python 버전은 AGENTS.md의 `### develop > 기술스택 > Python` 버전을 참조하여 결정한다. 기술스택 정보가 없으면 기본값 `python:3.11-slim`을 사용한다.
...
## 출력 형식
`docs/deploy/build-image-ai.md` 파일에 수행한 명령어를 포함하여 컨테이너 이미지 작성 과정을 단계별로 기록한다.
