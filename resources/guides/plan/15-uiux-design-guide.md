...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| 유저스토리 | `docs/plan/design/userstory.md` | Epic → 주요 화면 그룹, User Story → 화면 플로우, AC → 화면 상태/전환 변환 |
...
### 감정 저점 UX 대응
유저저니맵에서 감정 레벨 1-2인 구간의 화면에서는 다음 UX 보완 요소를 필수 포함:
- **진행 안내**: 복잡한 단계에서 진행률 표시, 단계 안내
- **에러 복구**: 실수 시 쉽게 이전 상태로 돌아갈 수 있는 경로
- **격려 메시지**: 작업 완료 시 긍정적 피드백 (예: "거의 다 됐어요!")
- **도움말**: 맥락에 맞는 인라인 도움말, 툴팁

### 터치포인트-화면 매핑
여정 맵의 각 터치포인트가 어떤 화면에 대응하는지 매핑 테이블을 작성:

| 고객 여정 단계 | 터치포인트 | 대응 화면 | 감정 레벨 | UX 보완 요소 |
|--------------|-----------|----------|----------|------------|
| {단계명} | {터치포인트} | {화면명} | {1-5} | {감정 레벨 2 이하 시 보완 요소 기술} |

> **원칙**: 여정 맵의 모든 터치포인트는 최소 1개 화면에 대응되어야 함. 대응 화면이 없는 터치포인트는 화면 추가를 검토할 것.
...
### 완료 체크리스트
- [ ] 디자인 원칙 5개 정의
- [ ] 정보 아키텍처 (사이트맵 + 네비게이션 구조) 작성
- [ ] 주요 화면 5개 이상 와이어프레임 (ASCII 아트)
- [ ] 컴포넌트 라이브러리 완전 정의 (버튼/폼/카드/네비게이션/피드백)
- [ ] 접근성 WCAG 2.1 AA 체크리스트 포함
- [ ] 스타일 가이드 구체적 작성 (HEX 코드, px 단위)
- [ ] Mobile First 반응형 설계 (모바일/태블릿/데스크톱)
- [ ] 인터랙션 디자인 정의 (애니메이션, 마이크로 인터랙션)
- [ ] 유저스토리와 일대일 매핑 확인
- [ ] 터치포인트-화면 매핑 테이블 작성
- [ ] 감정 저점(레벨 1-2) 화면에 UX 보완 요소 포함
- [ ] 이전 단계 산출물(유저스토리, 유저저니맵)과 일관성 확인

### 정량 기준
- 주요 화면 와이어프레임: 5개 이상
- 디자인 원칙: 5개
- 반응형 브레이크포인트: 3종 (모바일/태블릿/데스크톱)
- 컬러 팔레트: Primary/Secondary/Accent + 상태 컬러 4종
