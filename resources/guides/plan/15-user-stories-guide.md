...
| PRD | `docs/plan/design/PRD.md` | 기능 요구사항(MoSCoW)·범위(In/Out) → UFR 도출 기준, 비기능 요구사항(목표치) → 관련 UFR/NFR의 구현 태스크·수용기준 근거 |
...
비기능 요구사항의 목표치(성능·보안·사용성·확장성 수치)는 `docs/plan/design/PRD.md`를 단일 출처로 하며,
유저스토리 단계에서는 목표치를 재정의하지 않고 목표치를 달성하기 위한 **구현 태스크·수용 기준**으로 구체화함.

### 반영 방식

| PRD 비기능 요구사항 예시 | 유저스토리 반영 방식 |
|----------------------|-------------------|
| 성능 목표치 (예: API 응답 2초 이내) | 관련 UFR의 [검증 요구사항] 또는 인수기준에 성능 조건 추가 |
| 보안 목표치 (예: 개인정보 최소 수집) | 관련 UFR의 [입력 요구사항]/[검증 요구사항]에 보안 처리 항목 추가 |
| 사용성 목표치 (예: WCAG 2.1 AA 준수) | 관련 UFR의 [처리 결과]/체크리스트에 접근성 검증 항목 추가 |
| 확장성 목표치 (예: 동시 세션 1,000개) | 별도 NFR 스토리(`NFR-{서비스}-{번호}`)로 인프라/아키텍처 구현 태스크 정의 |

> **원칙**: PRD는 "무엇을 달성해야 하는가(목표치)"를, 유저스토리는 "어떻게 구현하는가(태스크·수용기준)"를 다룸.
...
- 마이크로서비스 구조로 명확히 그룹화
- PRD 비기능 요구사항을 목표치 재정의 없이 구현 태스크·수용기준으로 구체화
