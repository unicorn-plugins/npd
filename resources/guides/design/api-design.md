...
유저스토리와 시퀀스 설계서를 기반으로, 서비스별 독립적인 OpenAPI 3.0 명세를 작성하여 프론트엔드-백엔드 간 계약을 명확히 정의함.
...
| 산출물 | 파일 경로 |
|--------|----------|
| 서비스별 OpenAPI 명세 | `docs/design/api/{service-name}-api.yaml` |
...
- 외부 시퀀스 설계서/내부 시퀀스 설계서와 일관성 있게 선정
...
- **순차 처리 필요 시**: 무리한 병렬화보다는 안전한 순차 처리
...
info:
  ...
  version: 1.0.0
servers:
  - url: https://mock.swaggerhub.com/{org}/{service}/1.0.0
