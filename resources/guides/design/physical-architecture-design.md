...
11. 컴퓨팅 아키텍처|3.2 서비스별 리소스 할당|3.2.4 AI 서비스|AI Pipeline Pod의 CPU/Memory requests/limits 설정 (Python/FastAPI 기반, LLM API 호출 대기 시 메모리 고려), GPU 필요 시 노드 풀 분리 명시
...
3. 컴퓨팅 아키텍처|3.3 서비스별 리소스 할당|3.3.3 AI 서비스|AI Pipeline Pod HPA 설정 (LLM API 응답 지연 고려한 스케일링 임계값), GPU 노드 풀 구성 (해당 시)
