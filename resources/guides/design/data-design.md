...
| 산출물 | 파일 경로 |
|--------|----------|
| 데이터 설계서 | `docs/design/database/{service-name}.md` |
| ERD | `docs/design/database/{service-name}-erd.puml` |
| DB 스키마 스크립트 | `docs/design/database/{service-name}-schema.psql` |
...
- 데이터 설계서: `design/backend/database/{service-name}.md` (시작 부분에 '데이터설계 요약' 포함)
- ERD: `design/backend/database/{service-name}-erd.puml` (PlantUML)
- DB 스키마 스크립트: `design/backend/database/{service-name}-schema.psql` (실행 가능한 SQL)
- 캐시 DB 설계서: `design/backend/database/cache-db-design.md` (Redis database 할당표 포함)
