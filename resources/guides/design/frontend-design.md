...
## 목적
사용자 플로우 프리젠테이션과 백엔드 API 명세서를 기반으로 기술 관점의 프론트엔드 설계서(UI/UX, 스타일가이드, 정보 아키텍처, API 매핑)를 작성한다.
...
| 산출물 | 파일 경로 |
|--------|----------|
| UI/UX 설계서 | `docs/design/frontend/uiux-design.md` |
| 스타일 가이드 | `docs/design/frontend/style-guide.md` |
| 정보 아키텍처 | `docs/design/frontend/ia.md` (사이트맵 + 프로젝트 구조 트리 포함) |
| API 매핑 설계서 | `docs/design/frontend/api-mapping.md` (runtime-env.js 설정 + 화면-API 매핑표 포함) |


## 방법론

<설계원칙>
- 기술스택: HighLevel아키텍처정의서의 '### 6.1.2 프론트엔드 기술스택'과 일치 
- 사용자 플로우 프리젠테이션와 동일하게 설계
- 각 백엔드서비스 API명세서와 반드시 일치
- 모바일, 태블릿, 웹 화면 크기에 맞게 반응형으로 디자인

<설계순서>
- 준비:
  - 사용자 플로우 프리젠테이션 분석 및 이해
  - API 설계서: "[백엔드시스템]"섹션의 정보를 이용하여 API명세서를 'docs/design/api'에 다운로드하여 분석 및 이해
  - 화면요구사항 분석: "[요구사항]" 섹션을 읽어 화면 요구사항 이해

- 설계:
  - 1. **UI/UX 설계**
    - 1.1 UI프레임워크 선택: 프론트엔드 기술스택에 맞는 프레임워크 선택(MUI, Ant Design, Chakra UI, Mantine, React Bootstrap 등)
    - 1.2 화면목록 정의
    - 1.3 화면 간 사용자 플로우 정의
    - 1.4 화면별 상세 설계:
      - 1.4.1 상세기능
