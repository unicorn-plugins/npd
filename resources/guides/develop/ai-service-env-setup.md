...
AI 서비스 설계서를 기반으로 Python/FastAPI 프로젝트를 초기화하고, Router→Service→Repository 계층 구조와 LangChain 패턴이 적용된 기본 골격을 설정한다. 이 단계에서는 비즈니스 로직을 구현하지 않으며, 실행 가능한 프로젝트 구조 확립에 집중한다.
...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| AI 서비스 설계서 | `docs/design/ai-service-design.md` | 아키텍처 구조, LLM 제공자, 의존성 목록 확인 |
| AI 서비스 API 명세 | `docs/design/api/ai-*-api.yaml` | 라우터 엔드포인트 목록, 요청/응답 스키마 확인 |
| AI 서비스 클래스 설계 | `docs/design/class/ai-*.puml` | 서비스·클라이언트 클래스 시그니처 확인 |
| HighLevel 아키텍처 | `docs/design/high-level-architecture.md` | Python 버전, 포트, 컨테이너 프로파일 확인 |
...
- Python 버전: high-level-architecture.md 기술스택 기준
...
**LangChain 기반 의존성 (설계서에 LangChain 명시 시)**:
...
> `{port}`는 `high-level-architecture.md`에서 AI 서비스에 할당된 포트로 교체한다.
...
| 의존성 파일 | 설계서 LLM 제공자·LangChain 사용 여부 반영 |
...
- **포트 충돌 방지**: 서비스 포트는 `high-level-architecture.md`에서 AI 서비스에 할당된 포트를 사용한다. 다른 서비스와 겹치지 않도록 확인한다.

- **Python 버전 일치**: `pyproject.toml`의 Python 버전 제약과 `Dockerfile`의 베이스 이미지 버전을 `high-level-architecture.md` 기술스택과 일치시킨다.

- **LangChain 선택 기준**: 단순 LLM API 호출(단일 요청-응답)은 LangChain 없이 직접 구현한다. RAG, Function Calling, Agent, 다단계 추론 등 복합 패턴이 설계서에 포함된 경우에만 LangChain을 추가한다.
