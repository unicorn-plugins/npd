...
| 산출물 | 파일 경로 | 활용 방법 |
|--------|----------|----------|
| 종합 개발 계획서 | `docs/develop/dev-plan.md` | 백킹서비스 요구사항(섹션 3, 10-2) |
| 백킹서비스 상세 정보 | `docs/develop/dev-plan.md` 섹션 10-2 | DB 종류·스키마명, Redis 설정, MQ 필요 여부 |
...
| 확인 항목 | 확인 위치 | 판단 결과 |
|----------|----------|----------|
| DB 종류 및 서비스별 database 명 | `docs/develop/dev-plan.md` 섹션 10-2 | (작성) |
| Cache 사용 여부 및 설정 | `docs/develop/dev-plan.md` 섹션 10-2 | (작성) |
| 비동기 MQ 통신 존재 여부 | `docs/develop/dev-plan.md` 섹션 10-2 | (작성) |
| OpenAPI yaml 파일 목록 | `docs/design/api/*.yaml` | (작성) |

> Step 1의 "DB 종류" 판단 결과가 PostgreSQL이 아닌 경우, 아래 Step 1-1에서 해당 카테고리 가이드를 참조한다.

### Step 1-1. 제품별 카테고리 가이드 참조 (조건부)

Step 1에서 확인한 DB/MQ 종류가 기본 제품(PostgreSQL, RabbitMQ/Kafka)이 아닌 경우,
해당 카테고리 가이드를 참조하여 docker-compose 서비스 블록을 구성한다.
...
| 서비스 | 프로파일 | 포함 조건 |
|--------|---------|----------|
| postgres (또는 mysql/mariadb) | 없음 (기본 기동) | dev-plan.md 섹션 10-2. 백킹서비스 요구사항에 사용 시 포함 |
| redis | 없음 (기본 기동) | dev-plan.md 섹션 10-2. 백킹서비스 요구사항에 사용 시 포함 |
| rabbitmq (또는 kafka) | 없음 (기본 기동) | dev-plan.md 섹션 10-2에서 MQ 필요로 판정된 경우만 포함 |
...
**MQ 서비스** (dev-plan.md 섹션 10-2에서 MQ 필요로 판정된 경우만 추가)
...
services:
  ...
  # MQ는 dev-plan.md 섹션 10-2에서 MQ 필요로 판정된 경우에만 포함
  # --- RabbitMQ 사용 시 ---
  # rabbitmq:
  #   image: rabbitmq:3-management
  #   environment:
...
- [ ] MQ 서비스는 dev-plan.md 섹션 10-2에서 MQ 필요로 판정된 경우에만 포함됨
