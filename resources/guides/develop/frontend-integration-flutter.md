...
2. API 모델 동기화: 현행화된 OpenAPI 명세 기반 Dart 모델 클래스 갱신
...
Step 4-0에서 현행화된 OpenAPI 명세(`docs/design/api/*.yaml`)와 프론트엔드의 Dart 모델 클래스를 동기화한다.
Mock 개발 시 원본 명세 기반으로 작성한 모델이 실제 백엔드 구현과 다를 수 있으므로, 실제 API 연동 전에 반드시 수행한다.
