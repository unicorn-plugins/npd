...
| 고수준 아키텍처 | `docs/design/high-level-architecture.md` | 기술스택 자동 판별 기준 |
...
`docs/design/high-level-architecture.md`의 기술스택 섹션을 읽고 아래 키워드 매핑표로 플랫폼을 **자동 판별**한다.
사용자가 별도로 지정한 스택이 있으면 그것을 우선한다.

#### 판별 키워드 매핑표

| `high-level-architecture.md` 내 키워드 | 판별 결과 (`{PLATFORM}`) |
|-----------------------------------------|--------------------------|
...
```
<!-- ELIF PLATFORM == FLUTTER -->
`docs/design/frontend/style-guide.md`를 반드시 참조하여 ThemeData를 작성한다.
style-guide.md에 정의된 컬러·타이포그래피 값을 Flutter ThemeData로 변환한다.
