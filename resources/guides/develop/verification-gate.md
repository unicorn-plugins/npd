...
### 원칙 3: 도구가 없으면 SKIP이 아니라 설치 후 진행이다
- 필수 도구(SDK, CLI 등)가 설치되어 있지 않으면 해당 검증 항목은 **FAIL**이다
- "도구 미설치로 건너뜀"은 허용하지 않는다
- FAIL 시 **자동 설치를 시도**하고, 설치 완료 후 게이트를 재수행해야 한다
- 자동 설치가 불가능한 경우에만 수동 설치 방법을 안내한다
...
```bash
# 임시 파일 디렉토리 생성 (테스트 로그, Playwright 스크린샷 등)
mkdir -p .temp
```
- 모든 테스트 로그·Playwright 캡처 이미지는 프로젝트 루트의 `.temp/` 디렉토리에 생성한다
- `.temp/`는 `.gitignore`에 등록한다
...
if ! flutter --version 2>/dev/null; then
  echo "FAIL: flutter not installed — 자동 설치를 시도합니다"
  # 1. Flutter SDK 자동 설치
  git clone https://github.com/flutter/flutter.git -b stable ~/flutter 2>&1 | tail -3
  # 2. 현재 세션에 PATH 추가
  export PATH="$HOME/flutter/bin:$PATH"
  # 3. 쉘 프로파일에 PATH 영구 등록 (bash/zsh 모두 대응)
  SHELL_RC="$HOME/.bashrc"
  [ -f "$HOME/.zshrc" ] && SHELL_RC="$HOME/.zshrc"
  if ! grep -q 'flutter/bin' "$SHELL_RC" 2>/dev/null; then
    echo 'export PATH="$HOME/flutter/bin:$PATH"' >> "$SHELL_RC"
    echo "PATH 설정 추가됨: $SHELL_RC"
  fi
  # 4. 설치 확인
  flutter --version 2>&1 | head -1 || echo "FAIL: flutter 자동 설치 실패 — 수동 설치 필요 (https://docs.flutter.dev/get-started/install)"
fi
dart --version 2>/dev/null || echo "FAIL: dart not installed (Flutter SDK에 포함되어 있어야 함)"
<!-- ENDIF -->
...
1. 자동 설치가 가능한 도구(Flutter SDK 등)는 **즉시 설치를 시도**한다
2. 자동 설치 실패 또는 자동 설치가 불가능한 도구는 설치 가이드를 제시하고 사용자에게 설치 완료 후 재수행을 요청한다
...
#### 5. 백엔드 빌드 + 전체 단위 테스트
```bash
./gradlew clean build test 2>&1 | tee .temp/gradle-test-output.log
echo "Exit code: $?"
# 결과 요약
grep -E "tests completed|BUILD" .temp/gradle-test-output.log
```
...
#### 6. AI 서비스 테스트 (해당 시)
```bash
cd ai-service && pip install -r requirements.txt && pytest -v 2>&1 | tee .temp/pytest-output.log
echo "Exit code: $?"
```
- `pip install` 성공
...
curl -s -o /dev/null -w '%{http_code}' http://localhost:5173
```
- Flutter 모바일 전용 프로젝트는 빌드 성공으로 대체
<!-- ENDIF -->
- HTTP 200이어야 1차 PASS

**Playwright MCP 브라우저 렌더링 검증:**

MCP Playwright 도구로 실제 브라우저를 열어 프론트엔드 렌더링을 검증한다:

1. `browser_navigate("http://localhost:5173")` → 페이지 이동
2. `browser_snapshot()` → DOM 구조 확인
   - 주요 UI 요소(루트 컨테이너, 네비게이션 등)가 존재하는지 확인
   - body가 비어있으면 **FAIL**
3. `browser_console_messages()` → JavaScript 콘솔 에러 확인
   - error 타입 메시지가 있으면 **FAIL**
   - CORS 에러(`Access-Control-Allow-Origin`, `CORS policy`) 발견 시: 백엔드 CORS 설정에서 프론트엔드 Origin(`http://localhost:{포트}`)을 허용 목록에 추가해야 한다
4. `browser_take_screenshot()` → 렌더링 결과를 `.temp/` 디렉토리에 저장하고 시각적 확인
   - 빈 화면, 깨진 레이아웃이면 **FAIL**
5. `browser_close()` → 브라우저 종료

- 위 5단계 모두 정상이어야 렌더링 검증 PASS
- **스냅샷 출력과 스크린샷을 증거로 기록한다**
...
#### 3. 통합 테스트 자동 실행
```bash
./gradlew test --tests '*IntegrationTest' --continue 2>&1 | tee .temp/integration-test-output.log
echo "Exit code: $?"
# 결과 요약
grep -E "tests completed|BUILD" .temp/integration-test-output.log
```
...
**5-d. Playwright MCP 브라우저 통합 스모크 테스트**

프론트엔드 개발 서버가 실제 백엔드에 연결된 상태에서 MCP Playwright 도구로 브라우저를 열어 페이지 렌더링과 API 연동을 검증한다.

<!-- IF PLATFORM == REACT -->
```bash
# 프론트엔드 개발 서버 기동 (이미 기동 중이면 생략)
cd frontend && npm run dev &
sleep 10
```
<!-- ELIF PLATFORM == VUE -->
...
<!-- ELIF PLATFORM == FLUTTER -->
```bash
# Flutter Web 빌드 + 서빙 (이미 기동 중이면 생략)
cd frontend && flutter build web 2>&1 | tail -5
python3 -m http.server 5173 -d build/web &
sleep 3
```
- Flutter 모바일 전용 프로젝트는 5-c 빌드 성공으로 대체
<!-- ENDIF -->

MCP Playwright 도구로 검증한다:

1. `browser_navigate("http://localhost:5173")` → 메인 페이지 이동
2. `browser_snapshot()` → DOM 구조 확인
   - 주요 UI 요소(루트 컨테이너, 콘텐츠 영역)가 렌더링되었는지 확인
   - body가 비어있으면 **FAIL**
3. `browser_network_requests()` → API 연동 확인
   - `/api/` 경로 요청 중 5xx 응답이 있으면 **FAIL**
   - CORS preflight(OPTIONS) 요청 실패 시 **FAIL** — 백엔드 CORS 설정 확인 필요
   - 4xx 응답은 인증 미설정 등으로 허용 (별도 기록)
4. `browser_console_messages()` → JavaScript 콘솔 에러 확인
   - error 타입 메시지가 있으면 **FAIL**
   - CORS 에러 발견 시: 백엔드 CORS 허용 Origin에 프론트엔드 URL 추가 필요
5. `browser_take_screenshot()` → 렌더링 결과를 `.temp/` 디렉토리에 저장하고 시각적 확인
   - 빈 화면, 깨진 레이아웃이면 **FAIL**
6. `browser_close()` → 브라우저 종료
...
**4-a. 백엔드 단위 + 통합 테스트**
```bash
./gradlew clean build test 2>&1 | tee .temp/final-test-output.log
echo "=== Gradle Exit Code: $? ==="
grep -E "tests completed|BUILD" .temp/final-test-output.log
```
...
<!-- IF PLATFORM == REACT -->
```bash
cd frontend && npm ci && npm run build && npm test --if-present 2>&1 | tee .temp/frontend-test-output.log
echo "=== npm Exit Code: $? ==="
```
<!-- ELIF PLATFORM == VUE -->
```bash
...
flutter analyze --no-fatal-warnings 2>&1 | tee .temp/flutter-analyze-output.log
flutter test 2>&1 | tee .temp/flutter-test-output.log
flutter build web 2>&1 | tee .temp/flutter-build-output.log
echo "=== Flutter Build Exit Code: $? ==="
```
...
<!-- IF PLATFORM == REACT -->
```bash
cd e2e && npx playwright test 2>&1 | tee .temp/e2e-test-output.log
echo "=== E2E Exit Code: $? ==="
```
<!-- ELIF PLATFORM == VUE -->
```bash
...
if [ -f "e2e/api-e2e-test.sh" ]; then
  bash e2e/api-e2e-test.sh 2>&1 | tee .temp/e2e-test-output.log
  echo "=== E2E Exit Code: $? ==="
fi
if ls frontend/integration_test/*_test.dart 1>/dev/null 2>&1; then
  cd frontend && flutter test integration_test/ 2>&1 | tee .temp/flutter-integration-output.log
