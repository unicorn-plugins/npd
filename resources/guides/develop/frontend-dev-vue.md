...
// src/router/index.ts
const routes = [
  { path: '/login', component: LoginView, meta: { layout: 'auth', requiresAuth: false } },
  { path: '/profile/:nickname', component: PublicProfileView, meta: { layout: 'public', requiresAuth: false } },
  {
    path: '/',
...
```bash
# 이미지 생성 사용법
python {NPD_PLUGIN_DIR}/resources/tools/customs/general/generate_image.py \
  --prompt "생성할 이미지 설명" \
...
```vue
<!-- src/views/items/ItemFormView.vue -->
<script setup lang="ts">
...
onMounted(async () => {
  if (isEditMode.value && editId.value) {
    // 수정 모드: 기존 데이터를 API에서 조회하여 폼 초기값으로 설정
    const item = await store.fetchById(editId.value)
...
async function handleSubmit() {
  if (isEditMode.value && editId.value) {
    await store.updateItem(editId.value, formData.value)
  } else {
...
<template>
  <form @submit.prevent="handleSubmit">
    <h1>{{ isEditMode ? '수정' : '작성' }}</h1>
    <AppInput v-model="formData.title" label="제목" />
    <AppInput v-model="formData.content" label="내용" />
    <AppButton type="submit">{{ isEditMode ? '수정' : '작성' }}</AppButton>
  </form>
</template>
```
...
> **소셜 로그인(OAuth) 구현 시**: 공급자(GitHub/Google vs Kakao)에 따라 콜백 처리 방식이 다름.
> 전체 구현 패턴(버튼, 콜백 페이지, 라우터 등록)은 `{NPD_PLUGIN_DIR}/resources/guides/develop/oauth-guide.md` 섹션 4-2 참조.
