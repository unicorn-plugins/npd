...
| 종합 개발 계획서 (통합 맥락) | `docs/develop/dev-plan.md` | AI 서비스 관련 아키텍처 결정사항, 내부 흐름 |
| 행위 계약 테스트 | `test/design-contract/{ai-service-name}/*.spec.ts` | AI 서비스 행위 계약 |
...
- **런타임 에러 워크어라운드 금지**: 런타임 에러 발생 시 try-except로 삼키거나, 기능을 비활성화하거나, 하드코딩 값으로 대체하는 등의 우회 해결을 금지한다. 반드시 근본 원인을 분석하고 정상 동작하도록 수정한다.

- **서비스 기동 검증 필수**: `pytest` 통과만으로는 완료가 아니다. 실제 서비스를 기동하여 `/health`가 정상 응답을 반환하고, 최소 1개 핵심 API가 동작해야 한다.
