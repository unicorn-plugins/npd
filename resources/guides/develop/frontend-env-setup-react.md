...
프론트엔드 환경변수는 ROOT `.env` 파일에서 자동 생성한다.
...
기존 `dev`/`build` 명령어는 프로젝트의 프레임워크에 맞게 유지한다.
...
```bash
# tools/generate-runtime-env.sh 직접 실행하여 생성 확인
...
`runtime-env.js`의 `API_URL`이 `http://localhost:4010`으로 설정되어 있으면 자동으로 Prism Mock 서버로 요청이 전달된다.
