...
- {ENVIRONMENTS}: 배포 환경 목록 (쉼표 구분, 예: `dev,staging`)
...
> `{ENVIRONMENTS}`에 지정된 환경만 overlays 디렉토리 및 ArgoCD Application YAML을 생성.
> 아래는 `ENVIRONMENTS: dev,staging` 예시.
> 각 서비스 디렉토리에는 deployment.yaml, service.yaml 외에 해당 서비스의 configmap.yaml, secret.yaml 포함.
> 공용 리소스(공통 ConfigMap, Secret, ImagePull Secret, Ingress)는 common/ 디렉토리에 배치.
...
| 리소스 파일 | 설명 | 위치 |
|------------|------|------|
| `configmap.yaml` | 서비스별 환경설정 (환경변수, 설정값 등) | 각 서비스 디렉토리, common/ |
| `secret.yaml` | 서비스별 민감정보 (DB 접속정보 등) | 각 서비스 디렉토리, common/ |
| `secret-imagepull.yaml` | 컨테이너 이미지 레지스트리 인증 정보 (네임스페이스 단위 적용) | common/ |
| `ingress.yaml` | 외부 트래픽 라우팅 규칙 | common/ (백엔드 공용), 프론트엔드 base/ (전용) |
| `deployment.yaml` | 서비스 배포 정의 (이미지, 리소스, 볼륨 등) | 각 서비스 디렉토리 |
| `service.yaml` | 서비스 네트워크 노출 (ClusterIP/NodePort 등) | 각 서비스 디렉토리 |

> **common/ 디렉토리 역할**: 전 서비스가 공유하는 공용 리소스를 관리.
> common/은 Kustomize base 디렉토리로서 `base/kustomization.yaml`에서 디렉토리 단위로 참조됨.
> 서비스 디렉토리는 `base/kustomization.yaml`에서 개별 파일을 직접 resources로 참조하므로,
> 서비스 디렉토리에는 별도 kustomization.yaml이 불필요함.

> **ImagePull Secret 범위**: `secret-imagepull.yaml`은 네임스페이스 단위로 적용됨.
> 백엔드 common/에서 1회 생성 시, 같은 네임스페이스의 프론트엔드/AI 서비스도 사용 가능.
> 프론트엔드/AI가 별도 네임스페이스를 사용하는 경우 각각 생성 필요.

> **파일 네이밍**: 가이드에서는 리소스 유형별 일반명(`configmap.yaml`, `secret.yaml`)을 사용.
> 실제 프로젝트에서는 `cm-{서비스명}.yaml`, `secret-{서비스명}.yaml` 등 접두사 사용 가능.
> 네이밍 규칙은 프로젝트 컨벤션에 따라 결정.

#### kustomization.yaml 예시

**`base/kustomization.yaml`** (백엔드 시스템):
```yaml
...
resources:
  ...
  # 서비스별 리소스 (개별 파일 참조)
  - {서비스1}/configmap.yaml
  - {서비스1}/deployment.yaml
  - {서비스1}/secret.yaml
  - {서비스1}/service.yaml
  - {서비스2}/configmap.yaml
  - {서비스2}/deployment.yaml
  - {서비스2}/secret.yaml
  - {서비스2}/service.yaml
```
...
- 환경(env): `{ENVIRONMENTS}`에 지정된 환경만 생성 (기본값: `dev`)
- **총 생성 수**: (백엔드 1 + 프론트엔드 1 + AI 1) x {ENVIRONMENTS 개수}
...
- [ ] 매니페스트 레포지토리 구성 (Kustomize base/overlays 구조, 서비스별 ConfigMap/Secret 포함)
- [ ] 공용 리소스 구성 (common/ 디렉토리에 공통 ConfigMap, Secret, ImagePull Secret, Ingress 포함)
- [ ] ArgoCD Application YAML 서비스별 x `{ENVIRONMENTS}` 환경별 생성
...
- Kustomize 파일 생성 시 `commonLabels`는 deprecated이므로 `labels`를 사용한다
