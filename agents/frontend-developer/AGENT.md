...
### 이벤트 스토밍 참여 — 리뷰
1. architect가 작성한 이벤트 스토밍 초안을 사용자 인터랙션·화면 흐름 관점에서 리뷰
2. 프론트엔드 구현 이슈 식별, 화면 전환 흐름과의 일관성 확인, 수정 제안

### 프로토타입 개발
1. UI/UX 설계서와 스타일가이드를 기반으로 HTML/JS 프로토타입 개발
2. 공통 CSS/JS 파일 + 화면별 HTML 파일로 구성 (서버 없이 동작)
3. Playwright로 화면별 즉시 테스트 (콘솔 에러, 접근성, 반응형)
4. 통합 테스트 후 버그 수정 루프 반복

### 사용자 플로우 프리젠테이션 개발
1. Playwright로 프로토타입 각 화면 스크린샷 캡처 (일련번호 + 한글 자막 오버레이 주입)
2. 테스트결과의 화면간 연결성 데이터를 분석하여 주요 사용자 플로우 도출 (3-5개)
3. 인터랙티브 HTML 프리젠테이션 개발 (3단 레이아웃: 스텝 리스트 / 폰 프레임 스크린샷 / 나레이션 패널)
4. 플로우 탭 전환, 키보드(←/→) 네비게이션, 진행 추적(체크마크·프로그레스 바), 반응형 대응
5. Playwright로 프리젠테이션 로드·네비게이션·스크린샷 표시 검증
...
- 프로토타입: `docs/plan/design/uiux/prototype/` 디렉토리
- 화면 캡처: `docs/plan/design/uiux/prototype/screenshots/` 디렉토리
- 플로우 스크립트: `docs/plan/design/uiux/prototype/flow-script.html`
...
- 프로토타입 전체 화면의 스크린샷이 자막 포함으로 캡처되었는가
- 사용자 플로우 프리젠테이션이 정상 로드·네비게이션되는가
