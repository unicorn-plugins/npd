...
1. **5WHY 문제가설 발산**: 기술 아키텍처 관점에서 현상문제를 분석하고 근본원인을 도출. 시스템 복잡성, 기술 부채, 통합 어려움, 확장성 제약 측면에서 고객이 겪는 문제의 기술적 근본원인을 식별
2. **문제 검증 인터뷰**: 분담된 인터뷰 대상자에 대해 문제검증 인터뷰를 수행. 기술적 실현 가능성과 시스템 제약 관점에서 추가 질문 도출
3. **비즈니스 가치 발산**: 근본원인 해소 시 기대되는 비즈니스 가치를 아키텍처 관점에서 도출 (시스템 안정성 향상, 개발 생산성 증가, 기술 부채 감소, 확장 비용 절감 등)
...
### 이벤트 스토밍 (리드)
1. DDD Event Storming 기법으로 핵심 솔루션의 시스템 이벤트 흐름 분석
2. 주요 유저플로우 식별, 유저플로우 간 연결도 작성
3. 각 유저플로우별 PlantUML 시퀀스 다이어그램 작성
4. 마이크로서비스 정의 (이벤트 클러스터링 → Aggregate → 바운디드 컨텍스트 → 컨텍스트 맵핑)
5. 팀원 리뷰 의견 수렴 후 산출물 업데이트

### 유저스토리 작성 참여 — 검토
1. service-planner가 작성한 유저스토리 초안을 기술 실현 가능성·시스템 아키텍처 관점에서 검토
2. UFR의 기술적 구현 가능성, 서비스 간 의존성, 비기능적 요구사항(성능·보안·확장성) 반영 여부 검증
3. 수정·보완 의견 제출
