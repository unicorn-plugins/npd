...
배포 워크플로우 진입 전, 선택된 Cloud 서비스에 맞는 CLI가 설치·로그인되어 있는지 확인한다.

**확인 절차:**

1. 프로젝트 루트 `CLAUDE.md`의 `## NPD 워크플로우 상태 > ### design` 섹션에서 `CLOUD` 값을 읽는다 (AWS / Azure / GCP)
   - `CLOUD` 값이 없으면 사용자에게 직접 질문하여 결정
2. 사용자에게 Cloud CLI 사전 작업 완료 여부를 확인한다:

<!--ASK_USER-->
{"title":"Cloud CLI 사전 확인","questions":[
  {"question":"{CLOUD} CLI 설치 및 로그인이 완료되었나요?\n\n배포를 위해 Cloud CLI가 필요합니다.\n아직 완료하지 않으셨다면 아래 가이드를 참조하세요:\nhttps://github.com/unicorn-plugins/npd/blob/main/resources/guides/setup/prepare.md#cloud-cli-설치-및-로그인","type":"radio","options":["완료","아직 안 했음"]}
]}
<!--/ASK_USER-->

3. **완료** → VM 사전 확인으로 진행
4. **아직 안 했음** → 가이드 링크를 안내하고 스킬 종료. 완료 후 `/npd:deploy`로 재시작하도록 안내

## VM 사전 확인

컨테이너 실행 검증을 위해 {CLOUD}에 VM이 준비되어 있는지 확인한다.

<!--ASK_USER-->
{"title":"VM 사전 확인","questions":[
  {"question":"컨테이너를 실행할 VM이 {CLOUD}에 준비되어 있나요?\n\n아래 3가지 정보가 필요합니다:\n- SSH Key 파일 경로 (예: ~/.ssh/bastion-key)\n- OS 유저명 (예: azureuser, ec2-user, ubuntu)\n- VM IP 주소 (예: 4.230.5.6)\n\nVM이 준비되어 있나요?","type":"radio","options":["준비 완료","아직 없음"]}
]}
<!--/ASK_USER-->

1. **준비 완료** → 배포 환경 선택으로 진행
2. **아직 없음** → {CLOUD}에서 VM을 생성하고 SSH 접속이 가능한 상태로 준비한 뒤 `/npd:deploy`로 재시작하도록 안내 후 스킬 종료
