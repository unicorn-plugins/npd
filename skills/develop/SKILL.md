...
사용자에게 아래 내용을 텍스트로 표시하고 응답을 기다린다:

```
**[개발 범위 선택]**

HighLevel 아키텍처 정의서(14.1 개발 단계)에서 이번에 구현할 단계를 선택해 주세요.

{추출된 Phase 테이블 표시}

(복수 선택 가능, 번호로 입력 — 예: 1, 2)
1. {Phase 1: 주요산출물 — 마일스톤}
2. {Phase 2: 주요산출물 — 마일스톤}
N. {Phase N: ...}
```
...
각 단계 완료 후 승인을 받고 진행할까요, 자동으로 진행할까요?
1. 단계별 승인
2. 자동 진행
```
...
### Step 1. AI 개발 키트 컴파일 → Agent: architect (주도) + backend-developer + frontend-developer + ai-engineer (병렬 검증)  (`/ralplan` 활용)
...
#### Step 2-1. 백엔드 환경 구성 → Agent: backend-developer (`/ralph` 활용)
...
#### Step 2-2. 백킹서비스 + Mock 서버 로컬 구성 → Agent: backend-developer (`/ralph` 활용)
...
#### Step 2-3. 프론트엔드 프로젝트 초기화 → Agent: frontend-developer (`/ralph` 활용)
...
#### Step 2-4. AI 서비스 프로젝트 초기화 → Agent: ai-engineer (`/ralph` 활용)
...
#### Step 2-5. 실행 프로파일 작성 → Agent: backend-developer (`/ralph` 활용)
...
```
**[OAuth2 소셜 로그인 크리덴셜 요청]**

설계서에 OAuth2 소셜 로그인({식별된 Provider 목록})이 정의되어 있습니다.

통합 테스트를 위해 각 Provider의 Client ID와 Client Secret이 필요합니다.
...
크리덴셜을 입력해 주세요. (아직 미발급이면 '나중에 설정'을 입력하세요)
```
...
#### Step 3-1. 백엔드 API 구현 → Agent: backend-developer (`/ralph` 활용)
...
#### Step 3-2. Prism Mock API 기반 프론트엔드 개발 → Agent: frontend-developer (`/ralph` 활용)
...
#### Step 3-3. AI 서비스 구현 → Agent: ai-engineer (`/ralph` 활용)
...
#### Step 4-0. API 명세서 현행화 → Agent: backend-developer (`/analyze` 활용)

- **TASK**: 백엔드 Controller 클래스와 AI 서비스 엔드포인트를 분석하여 `docs/design/api/*.yaml`의 OpenAPI 명세서를 현행화한다.
  - **`@RequestMapping` API Group 반영 필수**: Controller 클래스 레벨의 `@RequestMapping("/api/v1/{resource}")` base path를 반드시 확인하여, 메서드 레벨 `@GetMapping`/`@PostMapping` 등의 경로와 결합한 **전체 경로**를 OpenAPI 명세에 반영한다. 메서드 레벨 경로만 추출하여 클래스 레벨 prefix를 누락하지 않도록 주의한다.
- **EXPECTED OUTCOME**: 현행화된 OpenAPI 명세서 (`docs/design/api/*.yaml`) + 변경 로그 (`docs/develop/api-update-log.md`)
- **선행 조건**: Step 3-1, Step 3-3 완료

#### Step 4-1. 프론트엔드 실제 API 연동 → Agent: frontend-developer (`/ralph` 활용)
...
#### Step 4-2. 백엔드 ↔ AI 서비스 연동 → Agent: backend-developer + ai-engineer (`/ralph` 활용)
...
#### Step 4-3. API 테스트 → Agent: qa-engineer (`/ralph` 활용)
...
#### Step 4-4. 브라우저 테스트 → Agent: qa-engineer (`/ralph` 활용)
