...
- **TASK**: 가이드의 검증 항목을 fail-fast 순서로 수행 (산출물 존재 → TODO/FIXME → API 명세 대조 → **API 응답 스키마 필드 대조(정적)** → FE 빌드+린트+테스트 → BE 빌드+테스트 → AI 테스트 → 서비스 독립 기동+헬스체크+Prism Mock 연동)
...
- **TASK**: 가이드의 검증 항목을 순서대로 수행 (Mock 잔존 확인 → 서비스 기동 → 통합 테스트 실행 → BE↔AI 연동 → FE→BE 실제 연동 검증 → **API 응답 스키마 동적 검증** → **Playwright MCP 유저 시나리오 전체 검증**)
