...
> **참조 문서**: `{PLUGIN_DIR}/resources/guides/develop/oauth-guide.md` — GitHub/Google/Kakao 앱 등록 절차, 필요 환경변수, Redirect URI 패턴, Spring Boot + 프론트엔드(React/Vue/Flutter) 구현 전체 패턴
...
- **PREV_ACTION**: Step 4(통합 연동) 진입 전에 반드시 아래 수행
  - 프로토타입 화면과 개발 화면 매핑 
...
- **TASK**:
  ...
  2. 서비스 기동:
     ...
     - 백엔드: `python3 tools/run-backend.py --config-dir . --delay 5`
       (tools/ 미존재 시: `{PLUGIN_DIR}/resources/tools/customs/general/run-backend.py` 사용)
...
2. **실행 도구 복사**: 아래 도구를 프로젝트 루트 `tools/`로 복사한다
   - `{PLUGIN_DIR}/resources/tools/customs/general/run-backend.py` → `tools/run-backend.py`
   - `{PLUGIN_DIR}/resources/tools/customs/general/run-frontend.py` → `tools/run-frontend.py`
