...
#### Step 4-0. API 명세서 현행화 → Agent: backend-developer (analyze 모드로 수행)

- **TASK**: 백엔드 Controller 클래스와 AI 서비스 엔드포인트를 분석하여 `docs/design/api/*.yaml`의 OpenAPI 명세서를 현행화한다.
  - **Controller/Router 탐색 (어노테이션/패턴 기반)**:
    ```bash
  ...
  - **DTO 추적 필수**: 각 controller 메서드가 참조하는 요청/응답 DTO 클래스를 추적하여 필드명, 타입, 필수 여부를 OpenAPI 명세의 schema에 정확히 반영한다.
