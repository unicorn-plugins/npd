...
#### Step 3-2 사전 확인: Gemini API Key 준비 (이미지 생성용)

Step 3-2 진입 전, 오케스트레이터가 프론트엔드 개발에 필요한 이미지 생성을 위해 Gemini API Key를 확인한다.
(위임된 에이전트는 사용자에게 질문할 수 없으므로, 이 확인은 반드시 오케스트레이터 레벨에서 수행)
...
5. '나중에 설정'을 선택한 경우:
   - `.env.example`에 `GEMINI_API_KEY=` 항목만 추가
   - Step 3-2 에이전트에 "Gemini API Key 미설정, 이미지 생성 불가 — 이미지가 필요한 곳은 CSS/SVG 기반 대체 구현" 지시를 포함
...
2. `.env` 파일에 해당 키가 이미 설정되어 있는지 확인한다
...
4. 사용자가 크리덴셜을 제공하면 `.env` 파일에 반영한다
5. '나중에 설정'을 선택한 경우:
   - `.env.example`에 해당 환경변수 키를 빈 값으로 추가
...
### Step 5. 브라우저 테스트 (3단계 오케스트레이터)

> **선행 조건**: Step 4-3(API 테스트) 전체 PASS 필수

브라우저 테스트는 **Phase 1(버그픽스)**, **Phase 2(개선)**, **Phase 3(Final Report)** 3단계로 진행한다.
역할 분리 원칙에 따라 테스트/검증하는 에이전트와 코드를 수정하는 에이전트를 분리한다.

#### Step 5 Phase 1: 버그픽스

##### Step 5-1-1. QA 테스트 → Agent: qa-engineer (ralph 모드로 수행)

- **GUIDE**: `{PLUGIN_DIR}/resources/guides/develop/test-on-browser.md`
- **TASK**: dev-plan.md의 비즈니스 시나리오(TC-01~N) + GAP 공통 시나리오(GAP-01~08 TC)를
  Playwright MCP 도구로 실제 브라우저에서 순서대로 실행
  1. 백킹서비스 + 백엔드 서비스 + 프론트엔드 개발 서버 기동 (AI 서비스 존재 시 함께 기동)
  2. 각 시나리오를 browser_navigate → browser_snapshot → browser_click/fill_form →
     browser_network_requests → browser_console_messages → browser_take_screenshot 순서로 수행
  3. 전체 페이지 스크린샷을 .temp/에 저장 (Phase 2에서 PO/SP가 활용)
- **EXPECTED OUTCOME**: `docs/develop/test/e2etest-{N}.md` (테스트 레포트)
- **주의**: 직접 에러를 고치지 않음. 테스트 결과 레포트만 작성

##### Step 5-1-2. 오케스트레이터 판정

- e2etest-{N}.md의 FAIL TC 존재 여부 확인
- **FAIL 없음** → Phase 2로 진행
- **FAIL 있음** → Step 5-1-3으로

##### Step 5-1-3. 오케스트레이터 → 개발자에게 수정 지시
...
##### Step 5-1-4. 오케스트레이터 → QA에게 재테스트 요청

- qa-engineer가 수정된 TC만 재실행
- **EXPECTED OUTCOME**: `docs/develop/test/e2etest-{N+1}.md`
- 전체 PASS → Phase 2 진행 / FAIL 잔존 → Step 5-1-3 복귀
- **루프 제한: 최대 3회**
  - 3회 후 Critical 잔존 → Step 5 FAIL 처리, 블로커로 에스컬레이션
  - 3회 후 Major/Minor만 잔존 → known issues로 문서화 후 Phase 2 진행

#### Step 5 Phase 2: 개선

##### Step 5-2-1. 오케스트레이터 → PO, SP에게 제품 검증 요청

- **입력**: QA의 전체 페이지 스크린샷 (.temp/tc-*.png) + 최종 e2etest 레포트
- PO와 SP는 병렬로 제품 검증 실행:
  - **PO** (product-owner): 스크린샷 + Playwright로 사용자 관점 순회
...
##### Step 5-2-2. 오케스트레이터 판정

- verify-po/sp-{N}.md의 Critical/Major 이슈 존재 여부 확인
- **이슈 없음** → Step 5-2-4(회귀 테스트)로
- **이슈 있음** → Step 5-2-3으로

##### Step 5-2-3. 오케스트레이터 → 개발자에게 개선 지시

- verify-po/sp-{N}.md의 이슈를 영향 파일 기준으로 분배
- 개발자들은 ralph 모드로 병렬 개선 실행
- 개선 완료 후 PO/SP 재검증 (최대 2회 루프)
- 2회 후 잔여 개선사항 → known improvements로 문서화 후 Step 5-2-4 진행

##### Step 5-2-4. 오케스트레이터 → QA에게 회귀 테스트 요청

- qa-engineer가 전체 TC 재실행 (개선으로 인한 회귀 버그 확인)
- **EXPECTED OUTCOME**: `docs/develop/test/e2etest-{N+1}.md`
- 회귀 버그 없음 → Step 5-3으로
- 회귀 버그 있음 → Phase 1 (버그픽스) 루프로 진입

#### Step 5 Phase 3: Final Report 작성

##### Step 5-3-1. 오케스트레이터가 Final Report 생성

Phase 1 + Phase 2 + 회귀 테스트 완료 후, 오케스트레이터가 직접 최종 보고서를 작성한다.
...
- [ ] Phase 1: 전체 TC PASS (비즈니스 TC + GAP TC)
- [ ] Phase 2: PO/SP Critical/Major 이슈 0건
- [ ] 회귀 테스트: 전체 TC PASS, 신규 버그 0건
- [ ] 전체 루프: Phase 1 ≤ 3회, Phase 2 ≤ 2회
...
- [ ] 브라우저 테스트 Phase 1 전체 TC PASS (비즈니스 TC + GAP TC)
- [ ] 브라우저 테스트 Phase 2 PO/SP Critical/Major 이슈 0건
- [ ] 브라우저 테스트 회귀 테스트 전체 TC PASS, 신규 버그 0건
- [ ] 브라우저 테스트 루프 제한 준수 (Phase 1 ≤ 3회, Phase 2 ≤ 2회)
- [ ] 브라우저 테스트 Final Report 생성 (`docs/develop/test/final-report.md`)
...
3. 브라우저 테스트 Phase 1 전체 PASS (Playwright MCP 기반 유저 시나리오 + GAP 패턴 검증)
4. 브라우저 테스트 Phase 2 PO/SP 제품 검증 완료 (Critical/Major 0건)
5. 브라우저 테스트 회귀 테스트 전체 PASS (신규 버그 0건)
6. 브라우저 테스트 루프 제한 준수 (Phase 1 ≤ 3회, Phase 2 ≤ 2회)
7. Final Report 생성 확인 (`docs/develop/test/final-report.md`)
