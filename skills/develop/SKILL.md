...
- **TASK**:
  ...
  2. 백킹서비스 + 백엔드 서비스 + 프론트엔드 개발 서버 기동 (AI 서비스 존재 시 함께 기동)
...
##### Step 5-M2. TC 목록 제시 + 사용자 테스트

dev-plan.md의 테스트 시나리오(TC-01~N)를 사용자에게 표시한다.

<!--ASK_USER-->
{"title":"브라우저 테스트 시작","questions":[
  {"question":"브라우저가 열렸습니다. 아래 TC 목록을 참고하여 직접 테스트해 주세요.\n\n{TC 목록 표시}\n\n이슈를 발견하면 알려주세요. 전체 OK이면 '테스트 완료'를 선택해 주세요.","type":"radio","options":["이슈 발견 — 수정 요청","테스트 완료 — 전체 OK"]}
]}
<!--/ASK_USER-->
...
사용자가 이슈를 보고하면:
1. 이슈 내용을 확인하여 영향 파일 기준으로 분배 (backend-developer / frontend-developer / ai-engineer)
2. 개발자 에이전트가 ralph 모드로 수정
3. 수정 완료 후 사용자에게 재확인 요청

<!--ASK_USER-->
{"title":"수정 확인","questions":[
  {"question":"수정이 완료되었습니다. 브라우저를 새로고침하여 확인해 주세요.","type":"radio","options":["수정 확인 — 다른 이슈 있음","수정 확인 — 전체 OK","수정 안됨 — 재수정 필요"]}
]}
<!--/ASK_USER-->

- **다른 이슈 있음** → 새 이슈 내용 받아 Step 5-M3 반복
- **전체 OK** → Step 5 완료, Step 6 진행
- **재수정 필요** → 추가 정보 받아 재수정 후 확인 반복
...
<!--ASK_USER-->
{"title":"제품 검증","questions":[
  {"question":"브라우저에서 전체 서비스를 둘러보며 UX/UI 개선점이 있으면 알려주세요.\n(디자인, 동선, 레이아웃, 성능 등)\n\n개선할 점이 없으면 '검증 완료'를 선택해 주세요.","type":"radio","options":["개선 요청","검증 완료 — 만족"]}
]}
<!--/ASK_USER-->
...
사용자가 개선을 요청하면:
1. 개선 내용을 영향 파일 기준으로 분배 (backend-developer / frontend-developer / ai-engineer)
2. 개발자 에이전트가 ralph 모드로 개선
3. 개선 완료 후 사용자에게 확인 요청

<!--ASK_USER-->
{"title":"개선 확인","questions":[
  {"question":"개선이 완료되었습니다. 브라우저를 새로고침하여 확인해 주세요.","type":"radio","options":["추가 개선 요청","만족 — 다음 단계로"]}
]}
<!--/ASK_USER-->

- **추가 개선 요청** → 새 피드백 받아 Step 6-M2 반복
- **만족** → Step 6 완료, Step 7 진행
