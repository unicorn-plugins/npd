...
- **EXPECTED OUTCOME**: 관찰결과.md — 관찰 {관찰수}회 결과 (고객경험 단계별 관찰 행동, 어려움, 특이사항, Pain Point 및 니즈 분석, 행동 패턴 분석)
...
- **EXPECTED OUTCOME**: 고객경험인터뷰결과.md — 인터뷰 {인터뷰수}명 개별 결과 (고객경험 단계별 행동·생각·긍정적 느낌·부정적 느낌)
...
- **EXPECTED OUTCOME**: 2개 파일 생성
  - 유저저니맵.md — 페르소나, 여정 맵 상세, 감정 곡선, 핵심 인사이트, 기회 영역
  - 유저저니맵.svg — 시각화된 여정 맵
...
- **EXPECTED OUTCOME**: 문제검증인터뷰결과취합.md — 인터뷰 결과 종합 취합 (문제별 통계, 근본원인 검증 결과, 핵심 인사이트)
...
- **EXPECTED OUTCOME**: 2개 파일 생성
  - 킹핀문제.md — 검증된 문제 리스트, 인과관계, 5가지 기준 평가, 킹핀 문제 선정
  - 문제해결방향성.md — Needs Statement, 상세 설명 (고객유형, 목적, 원하는 것)
...
- **EXPECTED OUTCOME**: 2개 파일 생성
  - 솔루션탐색.md — 팀원별 아이디어 표 (Big Idea/Little Win Idea/Crazy Idea)
  - 솔루션후보.md — 유사도 평가 후 수렴된 솔루션 후보 (각 후보 상세 설명)
...
- **EXPECTED OUTCOME**: 3개 파일 생성
  - 솔루션평가.md — 투표 결과 집계표
  - 솔루션우선순위평가.svg — 우선순위 매트릭스 SVG
  - 핵심솔루션.md — 선정된 핵심 솔루션 (3개 이하, 상세 설명 포함)
...
- **EXPECTED OUTCOME**: 서비스기획서스크립트.md 생성
  - 10-15장 슬라이드 스크립트
  - 각 장표 핵심 메시지 3개 이하
  - 장표 간 `---` 구분
...
- **EXPECTED OUTCOME**: 다수 파일 생성
  ...
  - {순번}-{유저플로우명}.puml — 각 유저플로우 시퀀스 다이어그램
...
- **EXPECTED OUTCOME**: userstory.md 생성
...
- **EXPECTED OUTCOME**: prototype/ 디렉토리에 파일 생성
...
- **EXPECTED OUTCOME**: 2가지 산출물 생성
  - {번호}-{화면명}.png — 자막 포함 화면별 스크린샷
  - flow-script.html — 사용자 플로우 인터랙티브 프리젠테이션
