...
- **GUIDE**: `resources/guides/plan/01-mvp-definition-guide.md`
- **TASK**: CLAUDE.md의 MVP 주제를 기반으로 비즈니스 도메인을 정의하고, MVP 후보를 시장 잠재력·사용자 pain points·실현 가능성·경쟁 우위 4가지 팩터로 평가하여 최종 MVP 주제를 확정
- **EXPECTED OUTCOME**: MVP정의.md — MVP 주제 정의, 선정 이유(4가지 팩터 평가표), 목표 비즈니스 도메인
...
- **GUIDE**: `resources/guides/plan/02-customer-analysis-guide.md`
- **TASK**: JTBD 프레임워크로 타겟 고객 유형을 정의하고 사용자 세그먼트(3-5개)별 인구통계·행동 패턴·현재 상황, 주요 Job과 원하는 결과를 도출
- **EXPECTED OUTCOME**: 고객분석.md — 타겟 고객 유형(JTBD 형식), 사용자 세그먼트 3-5개, Job to be Done
...
- **GUIDE**: `resources/guides/plan/03-market-research-guide.md`
- **TASK**: MVP 주제와 타겟 고객에 대해 시장 규모(TAM/SAM/SOM), 경쟁 환경(3-5개 경쟁사), 고객 트렌드, 규제 환경, SWOT 분석, 시장 진입 전략을 포함한 철저한 시장 조사를 수행
- **EXPECTED OUTCOME**: 시장조사.md — 시장 규모·성장, 경쟁 환경, 고객 트렌드, 규제 환경, SWOT, 시장 진입 전략
...
- **GUIDE**: `resources/guides/plan/04-customer-journey-stages-guide.md`
- **TASK**: MVP 주제와 고객유형을 기반으로 현재 고객이 겪는 경험 단계(5-7개)를 문제 인식부터 해결/관리까지 순차적으로 정의
- **EXPECTED OUTCOME**: 고객경험단계.md — 고객경험 단계(화살표 연결), 단계별 설명(주요 행동·긍정/부정 느낌·Pain Points), 단계 도출 근거
...
- **GUIDE**: `resources/guides/plan/05-customer-experience-guide.md`
...
- **GUIDE**: `resources/guides/plan/06-journey-mapping-guide.md`
...
- **EXPECTED OUTCOME**: 유저저니맵.md — 페르소나·여정 맵 상세·감정 곡선·핵심 인사이트·기회 영역 / 유저저니맵.svg — 시각화된 여정 맵

### Step 7. 문제 가설 정의 → 전체 팀 협업 (발산-수렴-선택 패턴) (`/oh-my-claudecode:ralph` 활용)

> **도메인 컨텍스트 주입**: `domain-expert` 호출 시 프로젝트의 `.npd/domain-context.yaml`을 읽어 프롬프트에 포함합니다.

- **GUIDE**: `resources/guides/plan/07-problem-hypothesis-guide.md`
...
- **EXPECTED OUTCOME**: 문제가설.md — 문제 3개, 5WHY 분석, 다층적 근본원인 검토, 핵심 근본원인 선정
...
- **EXPECTED OUTCOME**: 문제검증인터뷰결과.md — {문제검증인터뷰수}명 개별 인터뷰 결과(중요도·불편도·근본원인 동의여부)
...
- **EXPECTED OUTCOME**: 문제검증인터뷰결과취합.md — 인터뷰 결과 종합 취합(문제별 통계·근본원인 검증·핵심 인사이트)

#### Step 7-4. 문제가설 피보팅 (순차) → Agent: product-owner

- **TASK**: 인터뷰 결과 취합을 반영하여 중요도/불편도가 낮거나 동의율이 낮은 항목을 조정하고 새로 발견된 문제를 반영하여 문제가설을 수정
- **EXPECTED OUTCOME**: 문제가설.md — 피보팅된 문제가설(기존 파일 덮어쓰기), 변경 사항과 피보팅 근거를 파일 상단에 명시
...
- **GUIDE**: `resources/guides/plan/09-ideation-guide.md`
...
- **EXPECTED OUTCOME**: 솔루션탐색.md — 팀원별 아이디어 표 / 솔루션후보.md — 수렴된 솔루션 후보(각 후보 상세 설명)
...
- **GUIDE**: `resources/guides/plan/10-solution-selection-guide.md`
...
- **EXPECTED OUTCOME**: 솔루션평가.md — 투표 결과 집계표 / 솔루션우선순위평가.svg — 우선순위 매트릭스 SVG / 핵심솔루션.md — 선정된 핵심 솔루션(3개 이하, 상세 설명)
...
- **GUIDE**: `resources/guides/plan/11-business-modeling-guide.md`
...
- **EXPECTED OUTCOME**: 비즈니스모델.md — Lean Canvas 9영역 전체·가격 전략·수익 전망·비용 구조·Key Metrics·경쟁 매트릭스·GTM 전략·재무 계획

### Step 12. 발표자료 스크립트 → Agent: service-planner (`/oh-my-claudecode:ralph` 활용)

- **GUIDE**: `resources/guides/plan/12-presentation-guide.md`
- **TASK**: 비즈니스 모델을 기반으로 린캔버스 9영역·경쟁 분석·GTM 전략·재무 계획을 10-15장으로 구성하고 장표별 핵심 메시지 3개 이하·비주얼 제안을 포함한 발표자료 스크립트를 작성
- **EXPECTED OUTCOME**: 서비스기획서스크립트.md — 10-15장 슬라이드 스크립트(각 장표 핵심 메시지 3개 이하, 장표 간 `---` 구분, 비주얼 제안 포함)
...
- **GUIDE**: `resources/guides/plan/13-event-storming-guide.md`
...
- **EXPECTED OUTCOME**: userflow.puml — 유저플로우 연결도 / {순번}-{유저플로우명}.puml — 각 유저플로우 시퀀스 다이어그램
...
- **EXPECTED OUTCOME**: 리뷰의견.md — 팀원별 리뷰 의견(관점·의견·수정 제안)
...
- **EXPECTED OUTCOME**: 기존 `*.puml` 파일 업데이트(반영 사항 주석 추가), 리뷰반영결과.md — 반영/미반영 판단 근거(ai-engineer AI 기회 발굴 의견 별도 섹션)
...
- **EXPECTED OUTCOME**: userstory.md — 마이크로서비스별 유저스토리(최소 20개 UFR)·사용자 역할·Feature Story Map·우선순위 매트릭스·스프린트 계획·비기능 요구사항·Definition of Done·리스크
...
- **EXPECTED OUTCOME**: userstory-리뷰의견.md — 검토자별 의견(관점·의견·수정 제안)
...
- **EXPECTED OUTCOME**: `userstory.md` 업데이트(기존 파일 덮어쓰기), userstory-리뷰반영결과.md — 반영/미반영 판단 근거(최종 UFR 최소 20개 확인)
...
- **EXPECTED OUTCOME**: uiux.md — UI/UX 디자인 명세 전체 / style-guide.md — 스타일 가이드
...
- **EXPECTED OUTCOME**: prototype/ 디렉토리 — common.js·common.css·{2자리번호}-{한글화면명}.html
...
- **EXPECTED OUTCOME**: {번호}-{화면명}.png — 자막 포함 화면별 스크린샷 / flow-script.html — 사용자 플로우 인터랙티브 프리젠테이션
