...
**GitHub Actions + GCP GKE (Artifact Registry):**
```
[실행정보]
- CI_TOOL: GitHubActions
- CLOUD: GCP
...
- **에이전트 프롬프트 필수 지시**: "가이드의 Deploy 단계(kubectl apply)는 무시하세요. 대신 CI 파이프라인의 마지막 단계로 manifest repo image tag 업데이트를 포함하세요. ArgoCD 가이드(`deploy-argocd-cicd.md`)의 manifest repo 업데이트 스크립트(Jenkins: `Update Manifest Repository` stage, GitHub Actions: `update-manifest` job)를 참조하여 결합하세요."
...
- **EXPECTED OUTCOME**:
  - Jenkins: `deployment/cicd/Jenkinsfile`, `deployment/cicd/kustomize/*`, `deployment/cicd/config/*`
  - GitHub Actions: `.github/workflows/backend-cicd.yaml`, `.github/kustomize/*`, `.github/config/*`
...
- **TASK**: 가이드를 참조하여 CI/CD 파이프라인 파일 생성 (빌드+푸시+manifest tag 업데이트)
- **EXPECTED OUTCOME**:
  - Jenkins: `deployment/cicd/Jenkinsfile` (프론트엔드용), `deployment/cicd/kustomize/*`
  - GitHub Actions: `.github/workflows/frontend-cicd.yaml`, `.github/kustomize/*`
